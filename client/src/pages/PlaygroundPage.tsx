...
export default function PlaygroundPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setMessages([...newMessages, {
          role: 'assistant',
          content: `${t('Error')}: ${err.error?.message ?? t('Unknown error')}`,
        }])
        return
      }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      <PageHeader
        title={t('Playground')}
        description={t('Send a chat completion through the router and see which provider serves it.')}
        actions={
          <>
            <Select value={selectedModel} onValueChange={(v) => setSelectedModel(v ?? 'auto')}>
              ...
              <SelectContent>
                <SelectItem value="auto">{t('Auto (fallback chain)')}</SelectItem>
          ...
          {messages.length === 0 ? (
            <div className="flex items-center justify-center h-full text-center">
              <div className="space-y-2 max-w-sm">
                <p className="text-base font-medium">{t('Send a message to get started.')}</p>
                {language === 'zh' ? (
                  <p className="text-sm text-muted-foreground">
                    当前使用：<span className="text-foreground">{activeModelLabel}</span>。{t('Switch models in the selector above.')}
                  </p>
                ) : (
                  <p className="text-sm text-muted-foreground">
                    {t('Using')} <span className="text-foreground">{activeModelLabel}</span>. {t('Switch models in the selector above.')}
                  </p>
                )}
              </div>
            </div>
                    ...
                    {msg.meta && (
                      <div className="flex items-center gap-2 mt-2 flex-wrap text-[11px] opacity-70 tabular-nums">
                        ...
                        {msg.meta.fallbackAttempts != null && msg.meta.fallbackAttempts > 0 && (
                          <span>· {msg.meta.fallbackAttempts} {t(msg.meta.fallbackAttempts > 1 ? 'fallbacks' : 'fallback')}</span>
                        )}
                      </div>
                    )}
                  </div>
        ...
        <div className="border-t bg-background/50 p-3">
          <div className="flex gap-2 items-end">
            <textarea
              ...
              placeholder={t('Type a message… (⏎ to send, ⇧⏎ for newline)')}
              rows={1}
